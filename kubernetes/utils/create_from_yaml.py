...
def create_from_dict(
    k8s_client, data, verbose=False, namespace="default", action="apply", **kwargs
):
    """
    ...
    if "List" in data["kind"]:
        ...
        try:
            created = create_from_yaml_single_item(
                k8s_client, data, verbose, namespace=namespace, action=action, **kwargs
            )
            k8s_objects.append(created)
        except client.rest.ApiException as api_exception:
            api_exceptions.append(api_exception)
...
def create_from_yaml_single_item(
    k8s_client, yml_object, verbose=False, action="apply", **kwargs
):
    ...
    if action == "apply":
        ...
        if hasattr(k8s_api, "delete_namespaced_{0}".format(kind)):
            ...
            resp = getattr(k8s_api, "delete_namespaced_{0}".format(kind))(
                # body=yml_object, name=name, **kwargs
                body=None, name=name, **kwargs  # 我只能说这玩意填个None就完事了真是太敷衍了
            )
        else:
            kwargs.pop("namespace", None)
            resp = getattr(k8s_api, "delete_{0}".format(kind))(
                # body=yml_object, name=name, **kwargs
                body=None, name=name, **kwargs  # 同上
            )
        if verbose:
        ...
        raise ValueError(f"Unsupported action {action}, only 'apply' and 'delete' is supported!")
